...
export class VehicleEntryInstance {
    ...
    public update(timeStep: number): void {
        ...
        if (
            ...
            if (this.targetSeat && this.entryPoint) {
                this.character.enterVehicle(this.targetSeat, this.entryPoint)
            } else {
                console.warn(
                    'this.targetSeat o this.entryPoint es undefined, hay codigo que no se ejecutara',
                    { targetSeat: this.targetSeat, entryPoint: this.entryPoint }
                )
            }
        }
    }
